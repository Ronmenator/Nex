...
impl UiState {
    ...
    pub fn render_frame(&mut self) {
        ...
        match &mut self.renderer {
            ActiveRenderer::Wgpu(r) => {
                ...
                if let Some(root) = self.root {
                    Self::render_widget_generic(root, &self.widgets, &self.layout, r);
                }
                r.end_frame();
                r.present();
            }
            ActiveRenderer::Overlay(r) => {
                r.begin_frame(width, height);
                r.clear(Color::from_rgba(0, 0, 0, 0));
                if let Some(root) = self.root {
                    Self::render_widget_generic(root, &self.widgets, &self.layout, r);
                }
                r.end_frame();
                // No present — engine handles compositing
            }
    ...
    fn render_widget_generic(
        ...
        for child_handle in children {
            Self::render_widget_generic(child_handle, arena, layout, renderer);
        }
    }

    pub fn hit_test_at(&self, x: f32, y: f32) -> bool {
        self.layout.hit_test(x, y, &self.widgets).is_some()
    }

    pub fn overlay_renderer(&self) -> Option<&OverlayRenderer> {
        match &self.renderer {
            ActiveRenderer::Overlay(r) => Some(r),
            _ => None,
        }
    }

    pub fn set_overlay_renderer(&mut self, renderer: OverlayRenderer) {
        self.renderer = ActiveRenderer::Overlay(renderer);
    }
    ...
    pub fn handle_mouse_up(&mut self, x: f32, y: f32) -> Vec<PendingCallback> {
        let hit = self.layout.hit_test(x, y, &self.widgets);
    ...
    pub fn handle_text_input(&mut self, text: &str) -> Vec<PendingCallback> {
        let mut pending = Vec::new();
    ...
    pub fn handle_key_press(&mut self, key: &str) -> Vec<PendingCallback> {
        let mut pending = Vec::new();
        if key == "Backspace" {
            let focused: Option<i64> = self
                .widgets
...
pub fn fire_pending(pending: Vec<PendingCallback>) {
    for cb in pending {
        ui_debug_log(
            "fire_pending",
            "invoking callback",
