...
pub unsafe extern "C" fn nex_ui_app_run(app: i64) {
    ...
    match state.config.backend {
        ...
        Backend::Overlay => {
            // Overlay mode: UI is driven by the engine, not its own event loop.
            // Put state back so overlay FFI functions can use it.
            STATE.with(|cell| {
...
// ---------------------------------------------------------------------------
// Overlay: UI rendered on top of the 3D engine
// ---------------------------------------------------------------------------

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_init(width: i64, height: i64) {
    ...
    state.set_overlay_renderer(overlay_renderer::OverlayRenderer::new(w, h));
    STATE.with(|cell| {
        *cell.borrow_mut() = Some(state);
    });
}
...
#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_render(width: i64, height: i64) -> *const u32 {
    let w = width.max(1) as u32;
    let h = height.max(1) as u32;
    STATE.with(|cell| {
        let mut borrow = cell.borrow_mut();
        ...
        // Update dimensions in config so layout uses correct size
        state.config.width = w;
        state.config.height = h;
        state.render_frame();
        state
            .overlay_renderer()
            .map(|r| r.framebuffer().as_ptr())
            .unwrap_or(std::ptr::null())
    })
}

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_mouse_move(x: i64, y: i64) {
    let fx = f64::from_bits(x as u64) as f32;
    let fy = f64::from_bits(y as u64) as f32;
    with_state(|s| {
        s.handle_mouse_move(fx, fy);
    });
}

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_mouse_down(x: i64, y: i64) {
    let fx = f64::from_bits(x as u64) as f32;
    let fy = f64::from_bits(y as u64) as f32;
    with_state(|s| {
        s.handle_mouse_down(fx, fy);
    });
}

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_mouse_up(x: i64, y: i64) {
    let fx = f64::from_bits(x as u64) as f32;
    let fy = f64::from_bits(y as u64) as f32;
    let pending = with_state(|s| s.handle_mouse_up(fx, fy));
    app::fire_pending(pending);
}

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_key_char(c: i64) {
    // c is a char code
    if let Some(ch) = char::from_u32(c as u32) {
        let text = ch.to_string();
        let pending = with_state(|s| s.handle_text_input(&text));
        app::fire_pending(pending);
    }
}

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_key_name(name: *const c_char) {
    let key_name = cstr_to_string(name);
    let pending = with_state(|s| s.handle_key_press(&key_name));
    app::fire_pending(pending);
}

#[no_mangle]
pub unsafe extern "C" fn nex_ui_overlay_hit_test(x: i64, y: i64) -> i64 {
    let fx = f64::from_bits(x as u64) as f32;
    let fy = f64::from_bits(y as u64) as f32;
    with_state(|s| if s.hit_test_at(fx, fy) { 1i64 } else { 0i64 })
}
